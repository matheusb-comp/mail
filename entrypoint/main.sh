...
# Substitute env vars in the base db config file
sed -i 's/{{DB_URL}}/'"$DB_URL"'/g' /etc/postfix/pgsql.cf
sed -i 's/{{DB_HOSTS}}/'"$DB_HOSTS"'/g' /etc/postfix/pgsql.cf
sed -i 's/{{DB_USER}}/'"$DB_USER"'/g' /etc/postfix/pgsql.cf
sed -i 's/{{DB_PASS}}/'"$DB_PASS"'/g' /etc/postfix/pgsql.cf
